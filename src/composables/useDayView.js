...
function layoutColumns(entries) {
  ...
  for (const e of sorted) {
    if (cur.length && e.start.getTime() >= curEnd) {
      clusters.push(cur);
      cur = [];
      curEnd = -Infinity;
    }
...
export function useDayView() {
  const model = computed(() => {
    ...
    const endFracOf = e =>
      Math.min(24, (Math.min(e.end.getTime(), nextMidnightMs) - dayStartMs) / 3600000);
    ...
    const onCallTotal = entries.reduce(
      (s, e) => s + (isOnCallTask(e.taskName) ? inDayHours(e) : 0),
      0,
    );
    ...
    const stats = {
      ...
      loggedMandays:
        hasClickUp && totalLogged > 0 ? `${formatHours(totalLogged / mandayHours.value)} MD` : "",
    ...
    let minH = work.start;
    let maxH = Math.max(work.end, work.start + 1);
    ...
    const placeholders = laid
      .filter(e => e._slot !== undefined)
      .map(seed => {
    ...
    const nowLine =
      isToday && nowFrac >= minH && nowFrac <= maxH
