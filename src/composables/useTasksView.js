...
export function useTasksView() {
  const model = computed(() => {
    ...
    const excluded = new Set(
      (state.config.excludeStatuses || []).map(s => String(s).trim().toLowerCase()).filter(Boolean),
    );
    ...
    for (const t of state.tasks) {
      ...
      if (!g) {
        g = {
          key,
          name: titleCase(t.statusName),
          dotColor: t.statusColor,
          type: t.statusType,
          order: t.statusOrder,
          priority: prioritized.has(key),
          tasks: [],
        };
        byStatus.set(key, g);
      }
