...
const overnight = computed(
  () => !!start.value && !!end.value && toMinutes(end.value) <= toMinutes(start.value),
);
...
const canSave = computed(
  () => connected.value && !saving.value && !!task.value && durationMinutes.value > 0,
);
      ...
      <div v-if="task" class="lt-field">
        ...
        <div class="lt-picked">
          ...
          <div v-if="task.sprint || task.customId" class="lt-picked-sub">
            <span v-if="task.sprint" class="finder-sprint"
              ><span aria-hidden="true">🏃</span>{{ task.sprint }}</span
            >
